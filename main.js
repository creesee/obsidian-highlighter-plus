...
// WtyczkiKody/highlighterplus/main.ts
var main_exports = {};
__export(main_exports, {
  default: () => HighlighterPlus
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");
var import_state = require("@codemirror/state");
var import_view = require("@codemirror/view");
var defaultColors = ["#8f3d3d", "#8f7a3d", "#668f3d", "#3d8f52", "#3d8f8f", "#3d528f", "#663d8f", "#8f3d7a"];
var DEFAULT_HIGHLIGHTERS = defaultColors.map((color, index) => {
  return {
    id: (index + 1).toString(),
...
var DEFAULT_SETTINGS = { highlighters: [...DEFAULT_HIGHLIGHTERS] };
...
var HighlighterPlus = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    __publicField(this, "settings");
    __publicField(this, "ghostIds", /* @__PURE__ */ new Set());
  }
  async onload() {
  ...
  onunload() {
    var _a, _b;
    (_a = document.getElementById("hp-styles")) == null ? void 0 : _a.remove();
    (_b = document.getElementById("hp-settings-layout-css")) == null ? void 0 : _b.remove();
  }
  injectGlobalStyles() {
    ...
    let css = `
            ...
            /* UKRYWANIE ELEMENT\xD3W SYNTAXU */
            body .markdown-source-view.mod-cm6 .hp-hide {
                display: none !important;
            }

            /* ZABICIE EWENTUALNYCH NARO\u015ALI Z INNYCH MOTYW\xD3W */
...
`;
    this.settings.highlighters.forEach((hl) => {
      const generateRule = (profile, themeClass) => {
        const rgbaColor = hexToRgba(profile.color, profile.opacity);
        const textStyle = profile.useCustomTextColor ? `color: ${profile.textColor} !important; -webkit-text-fill-color: ${profile.textColor} !important;` : `color: inherit !important; -webkit-text-fill-color: inherit !important;`;
...
`;
  ...
  buildLivePreviewPlugin() {
    return import_view.ViewPlugin.fromClass(class {
      ...
      update(update) {
        if (update.docChanged || update.viewportChanged || update.selectionSet || update.transactions.some((tr) => tr.effects.some((e) => e.is(forceUpdateEffect)))) {
      ...
      buildDeco(view) {
        ...
        while ((match = regex.exec(text)) !== null) {
          ...
          if (isInside) {
            builder.add(start, end, import_view.Decoration.mark({ class: `hp-base hp-${id}` }));
          } else {
            const idLen = id.length + 3;
            builder.add(start, start + idLen, import_view.Decoration.mark({ class: "hp-hide" }));
            builder.add(start + idLen, end - 2, import_view.Decoration.mark({ class: `hp-base hp-${id}` }));
            builder.add(end - 2, end, import_view.Decoration.mark({ class: "hp-hide" }));
          }
        }
  ...
  processReadingMode(element) {
    element.querySelectorAll("mark").forEach((markEl) => {
  ...
  cycleHighlights(editor) {
    ...
    if (tgl.length === 0) {
      new import_obsidian.Notice("No active highlighters for this context.");
      return;
    }
    ...
    if (sel) {
      ...
      const line = editor.getLine(cursor2.line);
      let left = cursor2.ch;
      let right = cursor2.ch;
      while (left > 0 && /[\p{L}\p{N}_]/u.test(line[left - 1])) {
        left--;
      }
      while (right < line.length && /[\p{L}\p{N}_]/u.test(line[right])) {
        right++;
      }
      if (left < right) {
        const word = line.slice(left, right);
        editor.replaceRange(`==${firstId}=${word}==`, { line: cursor2.line, ch: left }, { line: cursor2.line, ch: right });
        editor.setSelection(
          { line: cursor2.line, ch: left + prefixDelta },
          { line: cursor2.line, ch: left + prefixDelta + word.length }
        );
      }
    }
  }
  ...
  async loadSettings() {
    ...
    if (data && data.highlighters) {
      data.highlighters = data.highlighters.map((hl) => {
        let migratedStyleType = hl.styleType || hl.light && hl.light.styleType || "background";
        if (!hl.light) {
          const prof = {
            color: hl.color || "#8f3d3d",
            opacity: hl.opacity !== void 0 ? hl.opacity : 0.5,
            useCustomTextColor: hl.useCustomTextColor || false,
            textColor: hl.textColor || "#ffffff"
          };
          return { id: hl.id, styleType: migratedStyleType, borderRadius: hl.borderRadius !== void 0 ? hl.borderRadius : 3, isToggleable: hl.isToggleable !== void 0 ? hl.isToggleable : true, toggleScope: hl.toggleScope || "all", light: { ...prof }, dark: { ...prof } };
        }
        if (hl.light && hl.light.styleType) delete hl.light.styleType;
        if (hl.dark && hl.dark.styleType) delete hl.dark.styleType;
        if (hl.borderRadius === void 0) hl.borderRadius = 3;
        if (hl.styleType === void 0) hl.styleType = migratedStyleType;
        if (hl.toggleScope === void 0) hl.toggleScope = "all";
        return hl;
      });
    }
...
var DeleteConfirmModal = class extends import_obsidian.Modal {
  constructor(app, hlId, onConfirm) {
    super(app);
    __publicField(this, "onConfirm");
    __publicField(this, "hlId");
    this.hlId = hlId;
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).addButton((b) => b.setButtonText("Cancel").onClick(() => this.close())).addButton((b) => b.setButtonText("Delete").setWarning().onClick(() => {
      this.onConfirm();
      this.close();
    }));
...
var RestoreDefaultsModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).addButton((b) => b.setButtonText("Cancel").onClick(() => this.close())).addButton((b) => b.setButtonText("Restore").setWarning().onClick(() => {
      this.onConfirm();
      this.close();
    }));
    this.scope.register([], "Enter", (e) => {
...
var HPSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    __publicField(this, "plugin");
    __publicField(this, "highlightIdToFlash", null);
    this.plugin = plugin;
  }
  display() {
    ...
    this.plugin.settings.highlighters.forEach((hl, idx) => {
      ...
      idInp.onchange = async () => {
        ...
        if (this.plugin.settings.highlighters.some((h, i) => h.id === cleanVal && i !== idx)) {
          new import_obsidian.Notice("This name is already taken!");
          idInp.value = hl.id;
          return;
        }
      ...
      const tglComp = new import_obsidian.ToggleComponent(tglWrapper).setValue(hl.isToggleable);
      ...
      const dupBtn = actionsWrapper.createEl("button", { text: "\u{1F4C4}", cls: "hp-hl-action-btn", attr: { title: "Duplicate" } });
      ...
      const delBtn = actionsWrapper.createEl("button", { text: "\u{1F5D1}\uFE0F", cls: "hp-hl-action-btn hp-hl-del-btn", attr: { title: "Delete" } });
      delBtn.onclick = () => {
        new DeleteConfirmModal(this.app, hl.id, async () => {
          ...
          this.app.workspace.iterateAllLeaves((leaf) => {
            ...
            if (leaf.view.getViewType() === "markdown") {
              ...
              if ((_a = view.editor) == null ? void 0 : _a.cm) view.editor.cm.dispatch({ effects: forceUpdateEffect.of(null) });
            }
          });
          setTimeout(async () => {
      ...
      const renderProfile = (colParent, profile, isLight) => {
        ...
        col.createDiv({ text: isLight ? "\u2600\uFE0F LIGHT THEME" : "\u{1F319} DARK THEME", cls: "hp-col-title" });
        ...
        opRange.oninput = () => {
          opVal.innerText = `${Math.round(parseFloat(opRange.value) * 100)}%`;
        };
        opRange.onchange = async () => {
          profile.opacity = parseFloat(opRange.value);
          updatePreview();
          await this.plugin.saveSettings();
        };
        ...
        new import_obsidian.ToggleComponent(txtTglWrapper).setValue(profile.useCustomTextColor).onChange(async (v) => {
          profile.useCustomTextColor = v;
          txtCp.style.visibility = v ? "visible" : "hidden";
          updatePreview();
          await this.plugin.saveSettings();
        });
        txtCp.onchange = async () => {
          profile.textColor = txtCp.value;
          updatePreview();
          await this.plugin.saveSettings();
        };
      };
    ...
    new import_obsidian.Setting(bottomActions).addButton((b) => b.setButtonText("Add highlighter").setCta().onClick(async () => {
      ...
      var _a;
      const settingApp = this.app.setting;
      ...
      if (tab && tab.searchComponent) {
        ...
        (_a = tab.updateHotkeyVisibility) == null ? void 0 : _a.call(tab);
      }
    ...
    new import_obsidian.Setting(supportSection).setName("Support development").setDesc("If you enjoy using this plugin and would like to support my work, please consider using the button on the right. Thank you! \u2615").addButton(
